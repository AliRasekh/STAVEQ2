...
class TemporalBlock(nn.Module):
    def __init__(
            self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., init_values=1e-5,
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, with_cp=False):
        super().__init__()
    ...
    def forward(self, x, residual=None):
        ...
        def _inner_forward(x, residual=None):
            x_norm = self.norm1(x)
            x_attn = self.temporal_attn(x_norm)
...
class StackedBlock(nn.Module):
    ...
    def forward(self, x, residual=None, return_attn=False):
        def _inner_forward(x, residual=None, return_attn=False):
            attn_outputs = {}
            if return_attn:
                attn_out, attn_weights = self.attn(self.norm1(x), return_attn=True)
                attn_outputs['spatial_attn'] = attn_weights
            else:
            ...
            if return_attn:
                temp_out, temp_attn_weights = self.temporal_attn(self.norm3(x_temp), return_attn=True)
                attn_outputs['temporal_attn'] = temp_attn_weights
            else:
                temp_out = self.temporal_attn(self.norm3(x_temp))

            x_temp = x_temp + self.drop_path3(self.ls3 * temp_out)
        ...
        if self.with_cp:
            return checkpoint.checkpoint(_inner_forward, x, residual, return_attn)
        else:
            return _inner_forward(x, residual=residual, return_attn=return_attn)
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., rank=16, lora_dropout=0.05):
        ...
        # self.lora_A_fc1 = ModuleDict({"default": nn.Linear(in_features, rank, bias=False)})
        # self.lora_B_fc1 = ModuleDict({"default": nn.Linear(rank, hidden_features, bias=False)})
        # self.lora_A_fc2 = ModuleDict({"default": nn.Linear(hidden_features, rank, bias=False)})
        # self.lora_B_fc2 = ModuleDict({"default": nn.Linear(rank, out_features, bias=False)})
        # self.lora_embedding_A = ParameterDict()
        # self.lora_embedding_B = ParameterDict()

        # nn.init.eye_(self.lora_A_fc1["default"].weight)
        # nn.init.eye_(self.lora_B_fc1["default"].weight)
        # nn.init.eye_(self.lora_A_fc2["default"].weight)
        # nn.init.eye_(self.lora_B_fc2["default"].weight)
    
    def forward(self, x):
        base_out1 = self.fc1(x)
        # lora_out1 = self.lora_B_fc1["default"](self.lora_A_fc1["default"](self.lora_dropout["default"](x)))
        # x = base_out1 + lora_out1
        x = base_out1
        x = self.act(x)
        x = self.drop1(x)
        
        base_out2 = self.fc2(x)
        # lora_out2 = self.lora_B_fc2["default"](self.lora_A_fc2["default"](self.lora_dropout["default"](x)))
        # x = base_out2 + lora_out2
        x = base_out2
        x = self.drop2(x)
        return x
...
class PretrainVisionTransformer_clean(nn.Module):
    ...
    def forward(self, x, mask=None, use_image=False, return_attn=False):
        # print(x.shape)
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if return_attn:
                x_joint, attn_info = blk(x_joint, residual=residual, return_attn=True)
                attentions_map.append(attn_info)
            else:
        ...
        if self.x_vis_only:
            ...
            if return_attn:
                return x_vis, x_pool_vis, None, None, attentions_map
            else:
